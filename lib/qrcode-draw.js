...
QRCodeDraw.prototype = {
  ...
  draw:function(canvas,text,options,cb){
    ...
    this.globalAlpha = options.globalAlpha || this.globalAlpha || 1;
    if (options.color) {
      options.color.hasOwnProperty('dark') && (this.color.dark = options.color.dark);
      options.color.hasOwnProperty('light') && (this.color.light = options.color.light);
    }
    //create qrcode!
    try{
      ...
      qr.make();
      if (options.size) {
        this.scale = scale = options.size / (qr.getModuleCount() + 2 * this.marginScaleFactor);
      }

      var margin = this.marginWidth();
      var currenty = margin;
      width = (0 && options.size) || (this.dataWidth(qr) + margin*2);
      ...
      ctx.globalAlpha = 1;
      if(this.foreground && fs.existsSync(this.foreground)) {
        var img = new Image();
        var data;
        var w2 = canvas.width>>2;
        var h2 = canvas.height>>2;
        if(this.foreground instanceof Buffer) {
          data = this.foreground;
        } else {
          data = fs.readFileSync(this.foreground);
        }
        img.src = data;
        ctx.drawImage(img, (canvas.width - w2)/2, (canvas.height - h2)/2, w2, h2);
      }
    } catch (e) {
      error = e;
    }
  ...
  resetCanvas:function(canvas,ctx,width){
    ...
    if(this.background && fs.existsSync(this.background)) {
      var img = new Image();
      var data;
      if(this.background instanceof Buffer) {
        data = this.background;
      } else {
        data = fs.readFileSync(this.background);
      }
      ctx.clearRect(0,0,canvas.width,canvas.height);
      img.src = data;
      ctx.globalAlpha = 0.5;
